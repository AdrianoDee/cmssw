...
*/
          ...
          for (uint32_t candIdx = 0; candIdx < numCandidates; ++candIdx) {
            ...
            if (verbose_)
              printf("Cluster direction (cPos - vertex):");
            if (verbose_)
            ...
            if (verbose_)
              printf("  jetPx = %.3f, jetPy = %.3f, jetPz = %.3f\n", jetPx, jetPy, jetPz);
            if (verbose_)
              printf("  jetPt = %.3f, jetEta = %.3f, jetPhi = %.3f\n\n", jetPt, jetEta, jetPhi);
      ...
      ALPAKA_FN_ACC void closestClusters(clusterProperties* clusterData,
                                         uint32_t clusterIdx,
                                         int pixelIdx,
                                         float& minDist,
                                         float& secondMinDist,
                                         unsigned meanExp) const {
      ...
      ALPAKA_FN_ACC void secondDistDiffScore(clusterProperties* clusterData,
                                             uint32_t clusterIdx,
                                             unsigned meanExp) const {
      ...
      template <typename TAcc, typename = std::enable_if_t<isAccelerator<TAcc>>>
      ALPAKA_FN_ACC void printDebug(TAcc const& acc,
      ...
      template <typename TAcc, typename = std::enable_if_t<isAccelerator<TAcc>>>
      ALPAKA_FN_ACC void storeOutputDigis(TAcc const& acc,
                                          const SiPixelDigisSoAConstView digiView,
                                          SiPixelDigisSoAView outputDigis,
                                          uint32_t moduleId,
                                          uint32_t clusterOffset,
      ...
      ALPAKA_FN_ACC void splitCluster(TAcc const& acc,
        ...
        float jetDirLocalX = geoclusterView.transformXX(clusterIdx) * jetPx +
                             geoclusterView.transformYX(clusterIdx) * jetPy +
                             geoclusterView.transformZX(clusterIdx) * jetPz;
        float jetDirLocalY = geoclusterView.transformXY(clusterIdx) * jetPx +
                             geoclusterView.transformYY(clusterIdx) * jetPy +
                             geoclusterView.transformZY(clusterIdx) * jetPz;
        float jetDirLocalZ = geoclusterView.transformXZ(clusterIdx) * jetPx +
                             geoclusterView.transformYZ(clusterIdx) * jetPy +
                             geoclusterView.transformZZ(clusterIdx) * jetPz;
        ...
        float expSizeX =
            expSizeXAtLorentzAngleIncidence_ + std::abs(expSizeXDeltaPerTanAlpha_ * (jetTanAlpha - tanLorentzAngles));
        ...
        for (uint32_t j = 0; j < static_cast<uint32_t>(digiView.metadata().size()); j++) {
          if (static_cast<uint32_t>(digiView.moduleId(j)) == moduleId) {
            if (static_cast<uint32_t>(digiView.clus(j)) == clusterOffset) {
              clusterPropertiesDevice[clusterIdx].originalpixels_x[clusterPropertiesDevice[clusterIdx].pixelCounter] =
                  digiView.xx(j);  // Copy x-coordinate from original pixel
              clusterPropertiesDevice[clusterIdx].originalpixels_y[clusterPropertiesDevice[clusterIdx].pixelCounter] =
                  digiView.yy(j);  // Copy y-coordinate from original pixel

              clusterPropertiesDevice[clusterIdx].originalpixels_ADC[clusterPropertiesDevice[clusterIdx].pixelCounter] =
                  digiView.adc(j);

              clusterPropertiesDevice[clusterIdx].originalpixels_rawIdArr[j] = digiView.rawIdArr(j);
              ClusterCharge = ClusterCharge + clusterPropertiesDevice[clusterIdx]
                                                  .originalpixels_ADC[clusterPropertiesDevice[clusterIdx].pixelCounter];
        ...
        if (split) {
          ...
          if (meanExp <= 1) {
            if (verbose_)
              printf("meanExp <= 1 writing cluster");
            storeOutputDigis(acc, digiView, outputDigis, moduleId, clusterOffset, clusterCounterDevice);
          } else {
            // Splitting the pixels and writing them for the current clusterIdx
            if (verbose_)
              printf("cluster has meanExp=%d\n", meanExp);

            uint32_t pixelsSize = 0;
            for (uint32_t j = 0; j < clusterPropertiesDevice[clusterIdx].pixelCounter; j++) {
              int sub = static_cast<int>(clusterPropertiesDevice[clusterIdx].originalpixels_ADC[j]) / chargePerUnit_ *
                        expectedADC / centralMIPCharge_;
              if (sub < 1)
                sub = 1;
              ...
              if (verbose_)
                printf("Splitting %d in [ %d , %d ], expected numb of clusters: %u original pixel (x,y) %d %d sub %d\n",
                       ...
                       pixelsSize + sub,
                       meanExp,
                       clusterPropertiesDevice[clusterIdx].originalpixels_x[j],
                       clusterPropertiesDevice[clusterIdx].originalpixels_y[j],
                       sub);

              // Iterate over the sub-clusters (split pixels)
              for (int k = 0; k < sub; k++) {
                if (k == sub - 1)
                  perDiv = clusterPropertiesDevice[clusterIdx].originalpixels_ADC[j] -
                           perDiv * k;  // Adjust for the last pixel

                // Write the new split pixels at the obtained index
                clusterPropertiesDevice[clusterIdx].pixels[pixelsSize] = j;
                clusterPropertiesDevice[clusterIdx].pixel_X[pixelsSize] =
                    clusterPropertiesDevice[clusterIdx].originalpixels_x[j];  // Copy x-coordinate from original pixel
                clusterPropertiesDevice[clusterIdx].pixel_Y[pixelsSize] =
                    clusterPropertiesDevice[clusterIdx].originalpixels_y[j];  // Copy y-coordinate from original pixel
                clusterPropertiesDevice[clusterIdx].pixel_ADC[pixelsSize] = perDiv;  // Assign divided charge (ADC)
                clusterPropertiesDevice[clusterIdx].rawIdArr[pixelsSize] =
                    clusterPropertiesDevice[clusterIdx].originalpixels_rawIdArr[j];  // Copy rawIdArr from original pixel
                pixelsSize++;
              }
            }
            ...
            while (!stop && remainingSteps > 0) {
              ...
              for (uint32_t j = 0; j < clusterPropertiesDevice[clusterIdx].pixelCounter; ++j) {
                ...
                for (unsigned int i = 0; i < meanExp; i++) {
                  ...
                  float distanceX = 1.f * clusterPropertiesDevice[clusterIdx].originalpixels_x[j] -
                                    clusterPropertiesDevice[clusterIdx].clx[i];
                  float distanceY = 1.f * clusterPropertiesDevice[clusterIdx].originalpixels_y[j] -
                                    clusterPropertiesDevice[clusterIdx].cly[i];
                  float dist = 0;
                  ...
                  for (unsigned int subpixel = 0; subpixel < pixelsSize; subpixel++, subpixel_counter++) {
                    if (subpixel < maxPixels) {
                      if (clusterPropertiesDevice[clusterIdx].pixels[subpixel] > static_cast<uint32_t>(pixel_index)) {
                        break;
                      } else if (clusterPropertiesDevice[clusterIdx].pixels[subpixel] !=
                        ...
                        for (unsigned int subcluster_index = 0; subcluster_index < meanExp; subcluster_index++) {
                          if (subcluster_index < maxSubClusters) {
                            ...
                            float clDest =
                                1.f / (clusterPropertiesDevice[clusterIdx].distanceMap[pixel_index][subcluster_index] +
                                       0.05f);
                        ...
                        clusterPropertiesDevice[clusterIdx].cls[cl] +=
                            clusterPropertiesDevice[clusterIdx].pixel_ADC[subpixel];
                        ...
                        if (verbose_)
                          printf("Pixel weight weightOfPixel[%d]=%.4f  cl=%d\n",
                                 subpixel_counter,
                                 clusterPropertiesDevice[clusterIdx].weightOfPixel[subpixel_counter],
                                 cl);
                      }
                    } else {
              ...
              if (verbose_)
                printf("Recomputing cluster centers.........\n");

              stop = true;
              for (unsigned int subcluster_index = 0; subcluster_index < meanExp; subcluster_index++) {
                if (subcluster_index < maxSubClusters - 1) {
                  if (std::abs(clusterPropertiesDevice[clusterIdx].clx[subcluster_index] -
                               clusterPropertiesDevice[clusterIdx].oldclx[subcluster_index]) > 0.01f)
                    stop = false;  // still moving
                  if (std::abs(clusterPropertiesDevice[clusterIdx].cly[subcluster_index] -
                               clusterPropertiesDevice[clusterIdx].oldcly[subcluster_index]) > 0.01f)
                    stop = false;
                  clusterPropertiesDevice[clusterIdx].oldclx[subcluster_index] =
                      clusterPropertiesDevice[clusterIdx].clx[subcluster_index];
                  clusterPropertiesDevice[clusterIdx].oldcly[subcluster_index] =
                      clusterPropertiesDevice[clusterIdx].cly[subcluster_index];
              ...
              for (unsigned int pixel_index = 0; pixel_index < pixelsSize; pixel_index++) {
                if (pixel_index < maxPixels - 1) {
                  ...
                  clusterPropertiesDevice[clusterIdx]
                      .clx[clusterPropertiesDevice[clusterIdx].clusterForPixel[pixel_index]] +=
                      clusterPropertiesDevice[clusterIdx].pixel_X[pixel_index] *
                      clusterPropertiesDevice[clusterIdx].pixel_ADC[pixel_index];
                  clusterPropertiesDevice[clusterIdx]
                      .cly[clusterPropertiesDevice[clusterIdx].clusterForPixel[pixel_index]] +=
                      clusterPropertiesDevice[clusterIdx].pixel_Y[pixel_index] *
                      clusterPropertiesDevice[clusterIdx].pixel_ADC[pixel_index];
                  clusterPropertiesDevice[clusterIdx]
                      .cls[clusterPropertiesDevice[clusterIdx].clusterForPixel[pixel_index]] +=
                      clusterPropertiesDevice[clusterIdx].pixel_ADC[pixel_index];
                }
              }
              for (unsigned int subcluster_index = 0; subcluster_index < meanExp; subcluster_index++) {
                if (subcluster_index < maxSubClusters - 1) {
                  if (clusterPropertiesDevice[clusterIdx].cls[subcluster_index] != 0) {
                    clusterPropertiesDevice[clusterIdx].clx[subcluster_index] /=
                        clusterPropertiesDevice[clusterIdx].cls[subcluster_index];
                    clusterPropertiesDevice[clusterIdx].cly[subcluster_index] /=
                        clusterPropertiesDevice[clusterIdx].cls[subcluster_index];
                  }
                  if (verbose_)
            ...
            if (verbose_)
              printf("Accumulate pixel with same cl...\n");
            int p = 0;
            ...
            for (int cl = 0; cl < (int)meanExp; cl++) {
              ...
              for (unsigned int j = 0; j < pixelsSize; j++) {
                if (j < maxPixels) {
                  if (clusterPropertiesDevice[clusterIdx].clusterForPixel[j] == cl and
                      clusterPropertiesDevice[clusterIdx].pixel_ADC[j] != 0) {
                    ...
                    for (unsigned int k = j + 1; k < pixelsSize; k++) {
                      if (k < maxPixels) {
                        if (clusterPropertiesDevice[clusterIdx].pixel_ADC[k] != 0 and
                            clusterPropertiesDevice[clusterIdx].pixel_X[k] ==
                                clusterPropertiesDevice[clusterIdx].pixel_X[j] and
                            clusterPropertiesDevice[clusterIdx].pixel_Y[k] ==
                                clusterPropertiesDevice[clusterIdx].pixel_Y[j] and
                            clusterPropertiesDevice[clusterIdx].clusterForPixel[k] == cl) {
                          if (verbose_)
                            printf("Resetting all sub-pixel for location %d, %d at index %d associated to cl %d\n",
                                   clusterPropertiesDevice[clusterIdx].pixel_X[k],
                                   clusterPropertiesDevice[clusterIdx].pixel_Y[k],
                                   k,
                                   clusterPropertiesDevice[clusterIdx].clusterForPixel[k]);

                          clusterPropertiesDevice[clusterIdx].pixel_ADC[j] +=
                              clusterPropertiesDevice[clusterIdx].pixel_ADC[k];
                    ...
                    for (unsigned int p = 0; p < pixelsSize; ++p) {
                      if (verbose_)
                        printf("index, x, y, ADC: %u, %d, %d, %d associated to cl %d\n",
                               p,
                               clusterPropertiesDevice[clusterIdx].pixel_X[p],
                               clusterPropertiesDevice[clusterIdx].pixel_Y[p],
                               clusterPropertiesDevice[clusterIdx].pixel_ADC[p],
                               clusterPropertiesDevice[clusterIdx].clusterForPixel[p]);

                      if (verbose_)
                        printf("Adding pixel %d, %d to cluster %d\n",
                               clusterPropertiesDevice[clusterIdx].pixel_X[j],
                               clusterPropertiesDevice[clusterIdx].pixel_Y[j],
                               cl);
                    }

                    clusterPropertiesDevice[clusterIdx].pixelsForCl_X[cl][p] =
                        clusterPropertiesDevice[clusterIdx].pixel_X[j];
                    clusterPropertiesDevice[clusterIdx].pixelsForCl_Y[cl][p] =
                        clusterPropertiesDevice[clusterIdx].pixel_Y[j];
                    clusterPropertiesDevice[clusterIdx].pixelsForCl_ADC[cl][p] =
                        clusterPropertiesDevice[clusterIdx].pixel_ADC[j];
                    clusterPropertiesDevice[clusterIdx].pixelsForCl_rawIdArr[cl][p] =
                        clusterPropertiesDevice[clusterIdx].rawIdArr[j];
                    p++;
            ...
            for (int cl = 0; cl < (int)meanExp; cl++) {
              ...
              for (unsigned int j = 0;
                   j < static_cast<uint32_t>(clusterPropertiesDevice[clusterIdx].pixelsForClCounter[cl]);
                   j++) {
                if (verbose_)
                  printf("pixelsForCl[cl][j].x=%d, pixelsForCl[cl][j].y=%d, pixelsForCl[cl][j].adc=%d\n",
                ...
                outputDigis.xx(idx + j) =
                    static_cast<uint16_t>(clusterPropertiesDevice[clusterIdx].pixelsForCl_X[cl][j]);
                outputDigis.yy(idx + j) =
                    static_cast<uint16_t>(clusterPropertiesDevice[clusterIdx].pixelsForCl_Y[cl][j]);
                outputDigis.adc(idx + j) =
                    static_cast<uint16_t>(clusterPropertiesDevice[clusterIdx].pixelsForCl_ADC[cl][j]);
                outputDigis.rawIdArr(idx + j) =
                    static_cast<uint32_t>(clusterPropertiesDevice[clusterIdx].pixelsForCl_rawIdArr[cl][j]);
    ...
    void runKernels(TrackingRecHitSoAView<TrackerTraits>& hitView,
      ...
      if (verbose_)
        std::cout << "\nGot geoclusterView.metadata().size()=" << geoclusterView.metadata().size()
                  << "\nExecuting with " << alpaka::getWorkDiv<alpaka::Grid, alpaka::Blocks>(MyworkDiv)[0u]
                  << " blocks and " << threadsPerBlock << " threads per block "
                  << " and " << alpaka::getWorkDiv<alpaka::Grid, alpaka::Threads>(MyworkDiv)[0u] << " threads in total"
                  << std::endl;
