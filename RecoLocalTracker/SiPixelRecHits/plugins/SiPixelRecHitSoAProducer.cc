...
// const edm::EDPutTokenT<TrackingRecHit2DGPU> tokenHitGPU_;
  ...
  std::unique_ptr<float[]> store32_;
  std::unique_ptr<uint16_t[]> store16_;
  std::unique_ptr<uint32_t[]> modules_;
      ...
      isPhase2_(iConfig.getParameter<bool>("isPhase2"))
       {
         ...
         startBPIX2_ = isPhase2_ ? phase2PixelTopology::layerStart[1] : phase1PixelTopology::layerStart[1];
...
for (auto const& dsv : hits) {
  ...
  store32[c + numberOfHits*static_cast<int>(TrackingRecHit2DCPU::Storage32::kPhiStorage)] = 0;
  store32[c + numberOfHits*static_cast<int>(TrackingRecHit2DCPU::Storage32::kLayers)] = 0;
...
// std::cout << "SiPixelRecHitSoAProducer" << __LINE__ << std::endl;
 ...
 std::cout << "SiPixelRecHitSoAProducer" << __LINE__ << " copy done." <<std::endl;
 std::copy(store16.data(), store16.data() + int(nHits_ * n16), store16_.get());
 std::cout << "SiPixelRecHitSoAProducer" << __LINE__ << " copy done." <<std::endl;
 std::copy(moduleStart_.data(), moduleStart_.data() + nMaxModules, modules_.get());
 std::cout << "SiPixelRecHitSoAProducer" << __LINE__ << " copy done." <<std::endl;
}
...
void SiPixelRecHitSoAProducer::produce(edm::Event& iEvent, edm::EventSetup const& iSetup) {
  ...
  if(onGPU_)
  {
    ...
    std::cout << "SiPixelRecHitSoAProducer" << __LINE__ << " copy done." <<std::endl;
    pixelhitconverter::SiPixelRecHitSoAProducerOnGPU algo(nHits_,startBPIX2_,isPhase2_);
    std::cout << "SiPixelRecHitSoAProducer" << __LINE__ << " copy done." <<std::endl;
    ctx.emplace(iEvent, tokenHitGPU_, algo.convertHitsAsync(store32_.get(),store16_.get(), modules_.get(), ctx.stream()));
  }
  else
  {
    std::cout << "SiPixelRecHitSoAProducer" << __LINE__ << " copy done." <<std::endl;
    pixelhitconverter::SiPixelRecHitSoAProducerOnCPU algo(nHits_,startBPIX2_,isPhase2_);
    std::cout << "SiPixelRecHitSoAProducer" << __LINE__ << " copy done." <<std::endl;
    iEvent.emplace(tokenHitCPU_, algo.convertHits(store32_.get(),store16_.get(),modules_.get()));
  }

  // wrap the buffer in a HostProduct
  auto hms = std::make_unique<HMSstorage>(std::move(modules_));
  ...
  // auto output = std::make_unique<TrackingRecHit2DCPU>(store32.data(),store16.data(), hmsp.get(), numberOfHits, hitsModuleStart[startBPIX2_], isPhase2_, theStream);
  // std::cout << "SiPixelRecHitSoAProducer" << __LINE__ << std::endl;
  //
  //
  // // int numberOfDetUnits = 0;
  ...
  //     std::cout << "SiPixelRecHitSoAProducer" << __LINE__ << std::endl;
  //     TrackingRecHit2DGPU outputGPU(store32ForGPU, store16.data(), hmsp.get(), numberOfHits, hitsModuleStart[startBPIX2_], isPhase2_, theStream);
