...
void sortDigis(Queue& queue, bool shuffle, const unsigned int N = 50000) {
  ...
  auto v_d = cms::alpakatools::make_device_view(queue, digis.view().moduleId(), N);
  auto v_h = cms::alpakatools::make_host_buffer<uint16_t[]>(queue, N);

  auto i_d = cms::alpakatools::make_device_view(queue, digis.view().sortedDigiIdx(), N);
  ...
  // 4000 < 2^12
  using DigisHisto = HistoContainer<uint16_t, 4000 + 1, -1, 12, uint32_t>;
