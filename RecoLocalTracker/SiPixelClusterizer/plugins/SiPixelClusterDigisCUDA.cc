...
void SiPixelClusterDigisCUDA::acquire(const edm::Event& iEvent,
  ...
  // std::memcpy(cpu_input.get(), x.data(), sizeof(uint16_t) * nDigis);
  //
  // cudaCheck(cudaMemcpyAsync(d_X.get(), cpu_input.get(), sizeof(uint16_t) * nDigis, cudaMemcpyHostToDevice, ctx.stream()));

  // std::cout << "Allor "<< d_X.get()[0] << std::endl;
  //
