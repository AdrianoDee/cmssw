...
namespace pixelgpudetails {
  ...
  void SiPixelRawToClusterGPUKernel::makeClustersAsync(bool isRun2,
...

    {
      ...
      // MUST be ONE block
      fillHitsModuleStart<<<1, 1024, 0, stream>>>(clusters_d.clusInModule(), clusters_d.clusModuleStart(),2048);
  ...
  //                                                      cudaStream_t stream)
    ...
    printf("%d\n",x[0]);
    printf("fill digis %d\n",__LINE__);
    gpuCalibPixel::calibDigisUpgrade<<<blocks, threadsPerBlock, 0, stream>>>(x,y,a,p,id,
                                                                      digis_d.xx(),
                                                                      digis_d.yy(),
                                                                      digis_d.adc(),
                                                                      digis_d.pdigi(),
      ...
      countModules<<<blocks, threadsPerBlock, 0, stream>>>(
          digis_d.moduleInd(), clusters_d.moduleStart(), digis_d.clus(), n);
      cudaCheck(cudaGetLastError());
 ...
 //printf("clusters_d.nClusters() = %d \n",clusters_d.nClusters());
      // MUST be ONE block
      fillHitsModuleStart<<<1, 1024, 0, stream>>>(clusters_d.clusInModule(), clusters_d.clusModuleStart(),1024);
