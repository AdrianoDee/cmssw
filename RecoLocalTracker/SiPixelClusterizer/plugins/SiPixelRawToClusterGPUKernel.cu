...
namespace pixelgpudetails {
  ...
  __global__ void fillHitsModuleStart(uint32_t const *__restrict__ clusInModule,
                                      ...
                                      uint32_t *__restrict__ nModules_Clusters, bool isUpgrade) {
  ...
  void SiPixelRawToClusterGPUKernel::makeClustersAsync(bool isRun2,
...

    {
      ...
      fillHitsModuleStart<<<1, 1024, 0, stream>>>(clusters_d.clusInModule(), clusters_d.clusModuleStart(), clusters_d.moduleStart(), nModules_Clusters_d.get(),false);
  ...
  void SiPixelRawToClusterGPUKernel::makeDigiClustersAsync(const SiPixelClusterThresholds clusterThresholds,
                                                           const uint16_t *id,
                                                           const uint16_t *x,
                                                           const uint16_t *y,
                                                           const uint16_t *a,
    ...
    /*
 	...
 	cudaCheck(cudaMemset(clusters_d.clusInModule(),0, sizeof(uint32_t)*gpuClustering::maxNumModules));// cudaMemcpyDefault, stream));
    ...
    countModules<<<blocks, threadsPerBlock, 0, stream>>>(
        digis_d.moduleInd(), clusters_d.moduleStart(), digis_d.clus(), nDigis);
    cudaCheck(cudaGetLastError());
    ...
    auto nModules_Clusters_d = cms::cuda::make_device_unique<uint32_t[]>(3, stream);
    // MUST be ONE block
    fillHitsModuleStart<<<1, 1024, 0, stream>>>(clusters_d.clusInModule(), clusters_d.clusModuleStart(), clusters_d.moduleStart(), nModules_Clusters_d.get(), true);
