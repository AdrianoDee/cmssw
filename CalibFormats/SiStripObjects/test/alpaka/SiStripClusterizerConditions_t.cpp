/**
   Simple test for the reco::SiStripClusterizerConditionsCollection data structure
...
#include "SiStripClusterizerConditions_t.h"

using namespace ALPAKA_ACCELERATOR_NAMESPACE;
using namespace ALPAKA_ACCELERATOR_NAMESPACE::reco;
// Run 3 values, used for testing
constexpr uint32_t n_dets = 150;
constexpr uint32_t n_strips = 20000;
constexpr uint32_t n_avs = 1500;
int main() {
  ...
  for (const auto& device : devices) {
    ...
    // Inner scope to deallocate memory before destroying the stream
    {
      ...
      SiStripClusterizerConditionsCollection strip_conds_d({{n_dets,n_strips,n_avs}}, queue);
