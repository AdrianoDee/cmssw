...
template <typename TrackerTraits>
void CAHitNtupletGeneratorKernelsCPU<TrackerTraits>::launchKernels(HitsOnCPU const &hh, TkSoA *tracks_d, cudaStream_t cudaStream) {
  ...
  kernel_fillNLayers<TrackerTraits>(tracks_d);
  ...
  if (nhits > 1 && this->params_.lateFishbone_) {
    gpuPixelDoublets::fishbone<TrackerTraits>(hh.view(), this->device_theCells_.get(), this->device_nCells_, this->isOuterHitOfCell_, nhits, true);
  }
}

template <typename TrackerTraits>
void CAHitNtupletGeneratorKernelsCPU<TrackerTraits>::classifyTuples(HitsOnCPU const &hh, TkSoA *tracks_d, cudaStream_t cudaStream) {
  ...
  // remove duplicates (tracks that share a doublet)
  kernel_fastDuplicateRemover<TrackerTraits>(this->device_theCells_.get(), this->device_nCells_, tracks_d, this->params_.dupPassThrough_);
  ...
  ++iev;
  kernel_print_found_ntuplets<TrackerTraits>(hh.view(), tuples_d, tracks_d, quality_d, this->device_hitToTuple_.get(), 100, iev);
#endif
}
