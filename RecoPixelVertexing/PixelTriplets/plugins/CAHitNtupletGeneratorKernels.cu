...
void CAHitNtupletGeneratorKernelsGPU<TrackerTraits>::launchKernels(HitsOnCPU const &hh, TkSoA *tracks_d, cudaStream_t cudaStream) {
  ...
  blockSize = 64;
  numberOfBlocks = (3 * this->params_.maxNumberOfDoublets_ / 4 + blockSize - 1) / blockSize;
  ...
  cudaCheck(cudaGetLastError());
  kernel_fillNLayers<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(tracks_d);
  cudaCheck(cudaGetLastError());
  ...
  kernel_countMultiplicity<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(
      tuples_d, quality_d, this->device_tupleMultiplicity_.get());
  cms::cuda::launchFinalize(this->device_tupleMultiplicity_.get(), cudaStream);
  kernel_fillMultiplicity<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(
      tuples_d, quality_d, this->device_tupleMultiplicity_.get());
  cudaCheck(cudaGetLastError());
...
void CAHitNtupletGeneratorKernelsGPU<TrackerTraits>::buildDoublets(HitsOnCPU const &hh, cudaStream_t stream) {
  ...
  this->device_isOuterHitOfCell_ = cms::cuda::make_device_unique<OuterHitOfCellContainer[]>(
      std::max(1, nhits - hh.offsetBPIX2()), stream);
  assert(this->device_isOuterHitOfCell_.get());

  this->isOuterHitOfCell_ = OuterHitOfCell{this->device_isOuterHitOfCell_.get(), hh.offsetBPIX2()};

  this->cellStorage_ = cms::cuda::make_device_unique<unsigned char[]>(
  ...
  this->device_theCellTracksContainer_ = (CellTracks *)(this->cellStorage_.get() + TrackerTraits::maxNumOfActiveDoublets *
                                                                                      sizeof(CellNeighbors));
  ...
  this->device_theCells_ = cms::cuda::make_device_unique<GPUCACell[]>(this->params_.maxNumberOfDoublets_, stream);
...
template <typename TrackerTraits>
void CAHitNtupletGeneratorKernelsGPU<TrackerTraits>::classifyTuples(HitsOnCPU const &hh, TkSoA *tracks_d, cudaStream_t cudaStream) {
  ...
  kernel_classifyTracks<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(tuples_d, tracks_d, this->params_.cuts_, quality_d);

  if (this->params_.lateFishbone_) {
    // apply fishbone cleaning to good tracks
    numberOfBlocks = this->nDoubletBlocks(blockSize);
    kernel_fishboneCleaner<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(
        this->device_theCells_.get(), this->device_nCells_, quality_d);
    cudaCheck(cudaGetLastError());
  }
  ...
  if (this->params_.doSharedHitCut_ || this->params_.doStats_) {
    ...
    kernel_countHitInTracks<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(
        tuples_d, quality_d, this->device_hitToTuple_.get());
    cudaCheck(cudaGetLastError());
    assert((this->hitToTupleView_.assoc == this->device_hitToTuple_.get()) &&
           (this->hitToTupleView_.offStorage == this->device_hitToTupleStorage_.get()) && (this->hitToTupleView_.offSize > 0));
    cms::cuda::launchFinalize(this->hitToTupleView_, cudaStream);
    cudaCheck(cudaGetLastError());
    kernel_fillHitInTracks<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(tuples_d, quality_d, this->device_hitToTuple_.get());
    cudaCheck(cudaGetLastError());
#ifdef GPU_DEBUG
    cudaCheck(cudaDeviceSynchronize());
#endif
  ...
  if (this->params_.doSharedHitCut_) {
    ...
    kernel_rejectDuplicate<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(
        tracks_d, quality_d, this->params_.minHitsForSharingCut_, this->params_.dupPassThrough_, this->device_hitToTuple_.get());

    kernel_sharedHitCleaner<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(hh.view(),
                                                                          tracks_d,
                                                                          quality_d,
    ...
    if (this->params_.useSimpleTripletCleaner_) {
      kernel_simpleTripletCleaner<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(
          tracks_d, quality_d, this->params_.minHitsForSharingCut_, this->params_.dupPassThrough_, this->device_hitToTuple_.get());
    } else {
      kernel_tripletCleaner<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(
          tracks_d, quality_d, this->params_.minHitsForSharingCut_, this->params_.dupPassThrough_, this->device_hitToTuple_.get());
    }
    cudaCheck(cudaGetLastError());
#ifdef GPU_DEBUG
  ...
  if (this->params_.doStats_) {
    numberOfBlocks = (std::max(nhits, int(this->params_.maxNumberOfDoublets_)) + blockSize - 1) / blockSize;
    kernel_checkOverflows<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(tuples_d,
  ...
  if (this->params_.doStats_) {
    // counters (add flag???)
    numberOfBlocks = (this->hitToTupleView_.offSize + blockSize - 1) / blockSize;
    kernel_doStatsForHitInTracks<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(this->device_hitToTuple_.get(), this->counters_);
    cudaCheck(cudaGetLastError());
    numberOfBlocks = (3 * TrackerTraits::maxNumberOfQuadruplets / 4 + blockSize - 1) / blockSize;
    kernel_doStatsForTracks<TrackerTraits><<<numberOfBlocks, blockSize, 0, cudaStream>>>(tuples_d, quality_d, this->counters_);
    cudaCheck(cudaGetLastError());
  }
#ifdef GPU_DEBUG
  cudaDeviceSynchronize();
  ...
  kernel_print_found_ntuplets<TrackerTraits><<<1, 32, 0, cudaStream>>>(
      hh.view(), tuples_d, tracks_d, quality_d, this->device_hitToTuple_.get(), 100, iev);
#endif
}
