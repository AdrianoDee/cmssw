...
namespace gpuVertexFinder {
  ...
  __global__ void loadTracks(pixelTrack::TrackSoAT<TrackerTraits> const* ptracks, ZVertexSoA* soa, WorkSpace* pws, float ptMin, float ptMax) {
    assert(ptracks);
    assert(soa);
...
template<typename TrackerTraits>
#ifdef __CUDACC__
  ZVertexHeterogeneous ProducerT<TrackerTraits>::makeAsync(cudaStream_t stream, pixelTrack::TrackSoAT<TrackerTraits> const* tksoa, float ptMin, float ptMax) const {
  ...
  ZVertexHeterogeneous ProducerT<TrackerTraits>::make(pixelTrack::TrackSoAT<TrackerTraits> const* tksoa, float ptMin, float ptMax) const {
