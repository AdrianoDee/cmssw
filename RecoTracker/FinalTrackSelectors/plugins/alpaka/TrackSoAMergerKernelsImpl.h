...
namespace ALPAKA_ACCELERATOR_NAMESPACE::trackSoAMergerKernels {
    ...
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
      ...
      for (uint32_t i : cms::alpakatools::uniform_elements(acc, track_view.nTracks())) {
        ...
        auto const parameterDuplicate = [&](uint32_t a, uint32_t b) {
          ...
          if (minPt <= 0.f or alpaka::math::abs(acc, track_view[a].pt() - track_view[b].pt()) >
                                    maxRelativePtDifference * minPt)
            return false;
        ...
        if (track_view[i].quality() != pixelTrack::Quality::dup) {
          for (uint32_t j = 0; j < track_view.nTracks(); ++j) {
            if (i == j or track_view[j].quality() == pixelTrack::Quality::dup)
              continue;

            const bool duplicateByHits = doSameHitsDuplicates and sameHits(i, j);
            const bool duplicateByParameters = doParamDuplicates and parameterDuplicate(i, j);
            if (!(duplicateByHits or duplicateByParameters) or !better(j, i))
              continue;

            if (bestParent == invalid or better(j, bestParent))
              bestParent = j;
          }
        }

        isDuplicateOf[i] = bestParent;
      }
    }
  };
  ...
  public:
    ALPAKA_FN_ACC void operator()(Acc1D const& acc,
                                  ::reco::TrackSoAView track_view,
                                  const uint32_t* isDuplicateOf) const {
      for (uint32_t i : cms::alpakatools::uniform_elements(acc, track_view.nTracks())) {
        if (isDuplicateOf[i] != track_view.nTracks())
