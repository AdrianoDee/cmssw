...
//
// This CPU-only EDProducer forms stubs exclusively from genuine hit pairs:
// for each TrackingParticle, find its RecHits on OT stacked modules,
// pair the lower/upper sensor hits, and compute stub quantities.
//
// The output StubsHost collection can be consumed by SiPixelRecHitsStubsMerger
// instead of the normal OTStubProducer, giving the CA a perfect set of stubs
// (zero fakes, no missed genuine pairs) to measure its upper-bound performance.
//
// NOTE: This producer does NOT apply any bend or pT cut - all genuine pairs
// are kept regardless of kinematics.
...
void TrueStubProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  // Signal vs PU breakdown for selected TPs
  {
    ...
    edm::LogPrint("TrueStubProducer") << "=== Signal vs PU Breakdown ===\n"
                                       << "  Signal TPs: " << nSignalTPs
                                       << " (with OT stacks: " << nSignalTPsWithStacks << ")\n"
                                       << "  PU TPs: " << nPUTPs
                                       << " (with OT stacks: " << nPUTPsWithStacks << ")";
  }
    ...
    for (const auto* tp : selectedTPs) {
      ...
      if (nDiagPrinted < 10) {
        edm::LogPrint("TrueStubProducer")
            << "  PU TP nStacks=0: pT=" << tp->pt() << " eta=" << tp->eta() << " pdgId=" << tp->pdgId()
            << " eventId=(" << tp->eventId().bunchCrossing() << "," << tp->eventId().event() << ")"
            << " nTrackerHits=" << nTrackerHits << " nTrackerLayers=" << nTrackerLayers
            << " nG4Tracks=" << nG4Tracks;
  ...
  // Two modes controlled by tpAwarePHitOnly_:
  //   true:  Only add P-hits from selected TPs that have a lower-sensor hit but no
  //          matching upper-sensor hit (couldn't form a genuine stub). This gives
  //          a truth-level PHitOnly set consistent with the selected TPs.
  //   false: Add ALL unused lower-sensor P-hits on PS modules, regardless of which
  //          particle produced them. This matches the normal pipeline behavior where
  //          every P-hit is visible to the CA.
  ...
  // CRITICAL: The BrokenLineFit assumes a 1:1 mapping between merged hit position
  // and input StubsSoA position (stubIdx = hitIndex - offsetStubs). The merger's
  // ScatterStubsToHitsKernel reorders stubs by detectorIndex (module) using atomic
  // counters that process input stubs sequentially (on CPU serial). If our input
  // is also module-sorted, the scatter preserves the 1:1 position mapping.
  // Without this, the BrokenLineFit accesses the wrong stub's innerHitIdx → crash.
  //
  // We build a single output sequence sorted by geomModIdx, interleaving genuine
  // stubs and PHitOnly entries per module.
  struct OutputEntry {
    uint32_t geomModIdx;
    bool isGenuine;
  ...
  std::stable_sort(outputOrder.begin(), outputOrder.end(),
                   [](const OutputEntry& a, const OutputEntry& b) { return a.geomModIdx < b.geomModIdx; });
  ...
  if (nStubs > 0) {
    ...
    for (uint32_t iStub = 0; iStub < nStubs; ++iStub) {
      ...
      if (entry.isGenuine) {
        ...
        // Position: lower (pixel) sensor for PS, averaged for SS
        if (isPS) {
          stubsView[iStub].xGlobal() = hitsView[lowerIdx].xGlobal();
          stubsView[iStub].yGlobal() = hitsView[lowerIdx].yGlobal();
          stubsView[iStub].zGlobal() = hitsView[lowerIdx].zGlobal();
          stubsView[iStub].xLocal() = hitsView[lowerIdx].xLocal();
          stubsView[iStub].yLocal() = hitsView[lowerIdx].yLocal();
          stubsView[iStub].xerrLocal() = hitsView[lowerIdx].xerrLocal();
          stubsView[iStub].yerrLocal() = hitsView[lowerIdx].yerrLocal();
        } else {
          stubsView[iStub].xGlobal() = 0.5f * (hitsView[lowerIdx].xGlobal() + hitsView[upperIdx].xGlobal());
          stubsView[iStub].yGlobal() = 0.5f * (hitsView[lowerIdx].yGlobal() + hitsView[upperIdx].yGlobal());
          stubsView[iStub].zGlobal() = 0.5f * (hitsView[lowerIdx].zGlobal() + hitsView[upperIdx].zGlobal());
          stubsView[iStub].xLocal() = hitsView[innerIdx].xLocal();
          stubsView[iStub].yLocal() = hitsView[innerIdx].yLocal();
          stubsView[iStub].xerrLocal() = hitsView[innerIdx].xerrLocal();
          stubsView[iStub].yerrLocal() = hitsView[innerIdx].yerrLocal();
        }

        float xg = stubsView[iStub].xGlobal();
        float yg = stubsView[iStub].yGlobal();
        float rStub = std::sqrt(xg * xg + yg * yg);
        stubsView[iStub].rGlobal() = rStub;
        ...
        // dPhiDr with parallax correction
        {
          ...
          float z_stub = stubsView[iStub].zGlobal();
          // tiltAngle = atan2(dz, dr) measured from radial axis, so sinTilt > 0 for +z tilt
          float denominator = cosTilt + sinTilt * z_stub / rStub;
          float dr_effective;
          ...
          float ri2 = ri * ri;
          float sig2phi_i = (ri2 > 0.0f) ? (sinphi_i * sinphi_i * hitsView[innerIdx].xGlobalErr() -
                                             2.0f * sinphi_i * cosphi_i * hitsView[innerIdx].xyGlobalErr() +
                                             cosphi_i * cosphi_i * hitsView[innerIdx].yGlobalErr()) /
                                                ri2
                                          : 0.0f;
          float sinphi_o = std::sin(phi_outer);
          float cosphi_o = std::cos(phi_outer);
          float ro2 = ro * ro;
          float sig2phi_o = (ro2 > 0.0f) ? (sinphi_o * sinphi_o * hitsView[outerIdx].xGlobalErr() -
                                             2.0f * sinphi_o * cosphi_o * hitsView[outerIdx].xyGlobalErr() +
                                             cosphi_o * cosphi_o * hitsView[outerIdx].yGlobalErr()) /
                                                ro2
                                          : 0.0f;
          float dphi_err = std::sqrt(sig2phi_i + sig2phi_o);
          stubsView[iStub].dPhiDrError() =
              (std::abs(dr_effective) > 1e-6f) ? dphi_err / std::abs(dr_effective) : 0.0f;
        }

        // pT estimate from 3-point circle
        {
          ...
          float ptEst = 0.0f;
          if (std::abs(h1) > eps && h3 > eps) {
            float num = std::sqrt(ri2 * ro2 * h3);
            float rho = num / (2.0f * std::abs(h1));
          ...
          stubsView[iStub].ptEst() = ptEst;
        }

        uint32_t caModuleIndex = nPixelModules + geomIdx;
        stubsView[iStub].detectorIndex() = static_cast<uint16_t>(caModuleIndex);
        stubsView[iStub].innerHitIdx() = innerIdx;
        stubsView[iStub].outerHitIdx() = outerIdx;
        stubsView[iStub].stubType() = isPS ? reco::StubType::PS : reco::StubType::SS;
        stubsView[iStub].flags() = reco::StubFlags::makeFlags(isBarrel, isFlat, true, layer);
        stubsView[iStub].pHitGroupId() = iStub;  // unique per stub (set to lowerIdx to re-enable grouping)
      } else {
        // --- PHitOnly entry ---
        const auto& ph = phitOnlyEntries[entry.sourceIdx];
        uint32_t hitIdx = ph.lowerSoAIdx;

        stubsView[iStub].xGlobal() = hitsView[hitIdx].xGlobal();
        stubsView[iStub].yGlobal() = hitsView[hitIdx].yGlobal();
        stubsView[iStub].zGlobal() = hitsView[hitIdx].zGlobal();
        stubsView[iStub].rGlobal() = hitsView[hitIdx].rGlobal();
        stubsView[iStub].iphi() = hitsView[hitIdx].iphi();
        stubsView[iStub].xLocal() = hitsView[hitIdx].xLocal();
        stubsView[iStub].yLocal() = hitsView[hitIdx].yLocal();
        ...
        stubsView[iStub].dPhiDrError() = 0.0f;
        stubsView[iStub].bend() = 0.0f;
        stubsView[iStub].ptEst() = 0.0f;

        uint32_t caModuleIndex = nPixelModules + geomIdx;
        stubsView[iStub].detectorIndex() = static_cast<uint16_t>(caModuleIndex);
        stubsView[iStub].innerHitIdx() = hitIdx;
        stubsView[iStub].outerHitIdx() = UINT32_MAX;
        stubsView[iStub].stubType() = reco::StubType::PHitOnly;
        stubsView[iStub].flags() = reco::StubFlags::makeFlags(isBarrel, isFlat, true, layer);
        stubsView[iStub].pHitGroupId() = iStub;  // unique per stub (set to hitIdx to re-enable grouping)
      }
...
void TrueStubProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<bool>("tpAwarePHitOnly", true)
      ->setComment("If true, PHitOnly entries are restricted to P-hits from selected TPs only. "
