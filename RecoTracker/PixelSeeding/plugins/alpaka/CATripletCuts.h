...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  struct TripletCuts {
    ...
    // dPhi same sign cut
    // ---------------------------------
    // This cut checks that dPhi12 and dPhi23 have the same sign, which is expected for real tracks in
    // the absence of significant multiple scattering.
    ...
    ALPAKA_FN_ACC ALPAKA_FN_INLINE static bool accept(
      ...
      auto countRej = [&](int cut) {
        if constexpr (std::is_same_v<pixelTopology::Phase2OTStubs, TrackerTraits>) {
          if (pipelineCounters)
            alpaka::atomicAdd(acc, &pipelineCounters[cut], 1u, alpaka::hierarchy::Blocks{});
        }
      };
#endif
      ...
      if constexpr (std::is_same_v<pixelTopology::Phase2OTStubs, TrackerTraits>) {
        ...
        // all following cuts require stubs, so if we don't have them we can skip them
        if (nStubs == 0)
          return true;
        ...
        float curvatureStubs = sum_weightsTimesCurv / sum_weights;
        float curvatureStubsErrSquared = 1.f / sum_weights;
