...
void OTStubAnalyzer::analyze(edm::Event const& iEvent, edm::EventSetup const& iSetup) {
  ...
  if (printRecHits_) {
    ...
    for (uint32_t i = 0; i < nToPrint; ++i) {
      ...
      bool isLower = hitsView[i].isLower();
      bool isFlipped = (detIdx < nGeomModules) ? geomView[detIdx].isFlipped() : false;
      // Physical designation: if not flipped, lower=inner; if flipped, lower=outer
      bool isInner = isFlipped ? !isLower : isLower;

      std::ostringstream msg;
      msg << "Hit " << i << ":"
          << " globalPos=(" << hitsView[i].xGlobal() << ", " << hitsView[i].yGlobal() << ", " << hitsView[i].zGlobal()
          << ")"
          << " r=" << hitsView[i].rGlobal() << " iphi=" << hitsView[i].iphi() << " localPos=(" << hitsView[i].xLocal()
          << ", " << hitsView[i].yLocal() << ")"
          << " localErr=(" << hitsView[i].xerrLocal() << ", " << hitsView[i].yerrLocal() << ")"
          << " detIdx=" << detIdx << " stackDetId=" << hitsView[i].stackDetId()
  ...
  if (printBendStubs_) {
    ...
    for (uint32_t i = 0; i < nToPrint; ++i) {
      ...
      msg << "Stub " << std::setw(2) << std::right << i << ": "
          << "pos=(" << std::setw(8) << stubsView[i].xGlobal() << ", " << std::setw(8) << stubsView[i].yGlobal() << ", "
          << std::setw(8) << stubsView[i].zGlobal() << ") "

          << "r=" << std::setw(8) << stubsView[i].rGlobal() << " iphi=" << std::setw(5) << std::right
          << stubsView[i].iphi() << " "

          << "localPos=(" << std::setw(8) << stubsView[i].xLocal() << ", " << std::setw(8) << stubsView[i].yLocal()
          << ") "

          << "bend=" << std::setw(8) << stubsView[i].bend() << " ptEst=" << std::setw(8) << stubsView[i].ptEst()
          << " dPhiDr=" << std::setw(8) << stubsView[i].dPhiDr() << " dPhiDrErr=" << std::setw(8)
          << stubsView[i].dPhiDrError() << " "

          << "detIdx=" << std::setw(6) << std::right << stubsView[i].detectorIndex() << " innerHit=" << std::setw(6)
          << stubsView[i].innerHitIdx() << " outerHit=" << std::setw(6) << stubsView[i].outerHitIdx()
          << " type=" << std::setw(2) << static_cast<int>(stubsView[i].stubType()) << " "

          << "width=" << std::setw(8) << (stubsView[i].bend() * stubsView[i].rGlobal()) << " ";
