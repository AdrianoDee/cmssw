...
namespace ALPAKA_ACCELERATOR_NAMESPACE {
  ...
  namespace testTrackingRecHitSoA {
    ...
    struct TestFillKernel {
      ...
      ALPAKA_FN_ACC void operator()(TAcc const& acc, ::reco::TrackingRecHitView soa, ::reco::HitModuleSoAView mods) const {
    ...
    struct ShowKernel {
      template <typename TAcc, typename = std::enable_if_t<isAccelerator<TAcc>>>
      ALPAKA_FN_ACC void operator()(TAcc const& acc, ::reco::TrackingRecHitConstView soa, ::reco::HitModuleSoAView mods) const {
        if (cms::alpakatools::once_per_grid(acc)) {
          printf("offsetBPIX = %d\n", soa.offsetBPIX2());
          printf("nHits = %d\n", soa.metadata().size());
