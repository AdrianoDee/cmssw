...
hltPhase2PixelTracksSoALowPt = cms.EDProducer('CAHitNtupletAlpakaPhase2OT@alpaka',
...
_hltPhase2PixelTracksSoALowPtNonCAExtended = cms.EDProducer('CAHitNtupletAlpakaPhase2@alpaka',
...
(hltPhase2LegacyTracking & hltPhase2LegacyTrackingPatatrackQuads).toReplaceWith(hltPhase2PixelTracksSoALowPt, _hltPhase2PixelTracksSoALowPtNonCAExtended)


def _exclude_OT_layers(hltPhase2PixelTracksSoALowPt, layers_to_exclude = [28, 29, 30]):
    keep_indices = []
    num_pairs = len(hltPhase2PixelTracksSoALowPt.geometry.pairGraph) // 2
    for i in range(num_pairs):
        a = hltPhase2PixelTracksSoALowPt.geometry.pairGraph[2*i]
        b = hltPhase2PixelTracksSoALowPt.geometry.pairGraph[2*i + 1]
    ...
    for i in keep_indices:
        new_pairGraph.extend([hltPhase2PixelTracksSoALowPt.geometry.pairGraph[2*i], hltPhase2PixelTracksSoALowPt.geometry.pairGraph[2*i+1]])

    hltPhase2PixelTracksSoALowPt.geometry.pairGraph[:] = new_pairGraph
    # Update all other lists in place
    hltPhase2PixelTracksSoALowPt.geometry.skipsLayers[:] = [hltPhase2PixelTracksSoALowPt.geometry.skipsLayers[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.phiCuts[:] = [hltPhase2PixelTracksSoALowPt.geometry.phiCuts[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.minInnerR[:] = [hltPhase2PixelTracksSoALowPt.geometry.minInnerR[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.maxInnerR[:] = [hltPhase2PixelTracksSoALowPt.geometry.maxInnerR[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.minOuterR[:] = [hltPhase2PixelTracksSoALowPt.geometry.minOuterR[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.maxOuterR[:] = [hltPhase2PixelTracksSoALowPt.geometry.maxOuterR[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.maxDR[:] = [hltPhase2PixelTracksSoALowPt.geometry.maxDR[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.minInnerZ[:] = [hltPhase2PixelTracksSoALowPt.geometry.minInnerZ[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.maxInnerZ[:] = [hltPhase2PixelTracksSoALowPt.geometry.maxInnerZ[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.minOuterZ[:] = [hltPhase2PixelTracksSoALowPt.geometry.minOuterZ[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.maxOuterZ[:] = [hltPhase2PixelTracksSoALowPt.geometry.maxOuterZ[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.minDZ[:] = [hltPhase2PixelTracksSoALowPt.geometry.minDZ[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.maxDZ[:] = [hltPhase2PixelTracksSoALowPt.geometry.maxDZ[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.ptCuts[:] = [hltPhase2PixelTracksSoALowPt.geometry.ptCuts[i] for i in keep_indices]
    hltPhase2PixelTracksSoALowPt.geometry.z0Cuts[:] = [hltPhase2PixelTracksSoALowPt.geometry.z0Cuts[i] for i in keep_indices]

#print("Using {} pair connections: {}".format(len(hltPhase2PixelTracksSoALowPt.geometry.pairGraph) // 2, hltPhase2PixelTracksSoALowPt.geometry.pairGraph))

# Stub-based tracking with OT stubs
from Configuration.ProcessModifiers.phase2CAStubs_cff import phase2CAStubs
from .hltPhase2PixelTracksSoALowPtWithStubs_cfi import hltPhase2PixelTracksSoALowPtWithStubs as _hltPhase2PixelTracksSoALowPtWithStubs
phase2CAStubs.toReplaceWith(hltPhase2PixelTracksSoALowPt, _hltPhase2PixelTracksSoALowPtWithStubs)
