...
//NB these specialization from the base class are needed since they are used by the constructor (in the base class)
// and if not explicilty defined the linker doesn't see them. FIXME: is there a cleaner way to do this that
// would still allow "partial specialization"?

template <typename Traits,typename TrackerTraits>
cms::cuda::host::unique_ptr<float[]> TrackingRecHit2DHeterogeneousT<Traits,TrackerTraits>::localCoordToHostAsync(cudaStream_t stream) const {
...
template <typename Traits,typename TrackerTraits>
void TrackingRecHit2DHeterogeneousT<Traits,TrackerTraits>::copyFromGPU(TrackingRecHit2DHeterogeneousT<cms::cudacompat::GPUTraits,TrackerTraits> const* input, cudaStream_t stream) {
  assert(input);
