...
//
// Converted from RecoTracker/PixelSeeding/test/StubValidationAnalyzer.cc (TFileService-based)
// to a DQMEDAnalyzer for integration into HLT validation workflows.
//
// Computes:
...
private:
  ...
  static constexpr const char* categoryTitles_[NCategories] = {
      "Barrel Flat", "Barrel Tilted", "Forward+", "Forward-"};
  ...
  static const char* perLayerCategoryName(int idx) {
    static const char* names[NPerLayerCategories] = {"B1_flat",  "B1_tilted", "B2_flat",  "B2_tilted",
                                                     "B3_flat",  "B3_tilted", "B4_flat",  "B4_tilted",
                                                     "B5_flat",  "B5_tilted", "B6_flat",  "B6_tilted",
                                                     "FwdP_D1",  "FwdP_D2",   "FwdP_D3",  "FwdP_D4",
                                                     "FwdP_D5",  "FwdN_D1",   "FwdN_D2",  "FwdN_D3",
                                                     "FwdN_D4",  "FwdN_D5"};
    return names[idx];
  }
  ...
  static const char* perLayerCategoryTitle(int idx) {
    static const char* titles[NPerLayerCategories] = {
        ...
        "Forward+ Disk 1",  "Forward+ Disk 2",  "Forward+ Disk 3",  "Forward+ Disk 4",
        "Forward+ Disk 5",  "Forward- Disk 1",  "Forward- Disk 2",  "Forward- Disk 3",
...
void Phase2OTValidateVectorHitStyleStub::bookHistograms(DQMStore::IBooker& iBooker,
  ...
  h_fake_per_layer_ =
      iBooker.book1D("fake_per_layer", "Non-genuine stubs per layer;Layer;Stubs", 11, 0.5, 11.5);
  for (int i = 1; i <= 6; ++i) {
  ...
  for (int c = 0; c < NCategories; ++c) {
    ...
    h_genuine_per_layer_cat_[c] = iBooker.book1D(
        "genuine_per_layer_" + std::string(categoryNames_[c]),
        "Genuine stubs per layer (" + std::string(categoryTitles_[c]) + ");Layer;Stubs",
        nLayers,
        0.5,
        nLayers + 0.5);
    h_total_per_layer_cat_[c] = iBooker.book1D(
        "total_per_layer_" + std::string(categoryNames_[c]),
        "Total stubs per layer (" + std::string(categoryTitles_[c]) + ");Layer;Stubs",
        nLayers,
        0.5,
        nLayers + 0.5);
    h_fake_per_layer_cat_[c] = iBooker.book1D(
        "fake_per_layer_" + std::string(categoryNames_[c]),
  ...
  h_genuine_matched_tp_pt_ = iBooker.book1D(
      "genuine_matched_tp_pt",
      "Matched TP p_{T} for genuine stubs;p_{T} [GeV];Genuine stubs",
      200,
      0,
      100);
  ...
  for (int c = 0; c < NCategories; ++c) {
    h_eff_denom_pt_cat_[c] = iBooker.book1D(
        "eff_denom_pt_" + std::string(categoryNames_[c]),
        "Efficiency denominator (" + std::string(categoryTitles_[c]) + ");p_{T} [GeV];RecHits",
        100,
        0,
        50);
    h_eff_numer_pt_cat_[c] = iBooker.book1D(
        "eff_numer_pt_" + std::string(categoryNames_[c]),
  ...
  h_eff_numer_eta_all_ =
      iBooker.book1D("eff_numer_eta_all", "Efficiency numerator (all);#eta;RecHits", 100, -4.2, 4.2);

  for (int c = 0; c < NCategories; ++c) {
    h_eff_denom_eta_cat_[c] = iBooker.book1D(
        "eff_denom_eta_" + std::string(categoryNames_[c]),
        "Efficiency denominator (" + std::string(categoryTitles_[c]) + ");#eta;RecHits",
        100,
        -4.2,
        4.2);
    h_eff_numer_eta_cat_[c] = iBooker.book1D(
        "eff_numer_eta_" + std::string(categoryNames_[c]),
  ...
  for (int i = 0; i < NPerLayerCategories; ++i) {
    ...
    h_eff_numer_pt_perlayer_[i] = iBooker.book1D(
        "eff_numer_pt_" + name, "Efficiency numerator (" + title + ");p_{T} [GeV];RecHits", 100, 0, 50);
  ...
  h_fake_numer_pt_all_ =
      iBooker.book1D("fake_numer_pt_all", "Fake rate numerator (all);p_{T} [GeV];Stubs", 100, 0, 50);

  for (int c = 0; c < NCategories; ++c) {
    h_fake_denom_pt_cat_[c] = iBooker.book1D(
        "fake_denom_pt_" + std::string(categoryNames_[c]),
        "Fake rate denominator (" + std::string(categoryTitles_[c]) + ");p_{T} [GeV];Stubs",
        100,
        0,
        50);
    h_fake_numer_pt_cat_[c] = iBooker.book1D(
        "fake_numer_pt_" + std::string(categoryNames_[c]),
  ...
  h_fake_numer_eta_all_ =
      iBooker.book1D("fake_numer_eta_all", "Fake rate numerator (all);#eta;Stubs", 100, -4.2, 4.2);

  for (int c = 0; c < NCategories; ++c) {
    h_fake_denom_eta_cat_[c] = iBooker.book1D(
        "fake_denom_eta_" + std::string(categoryNames_[c]),
        "Fake rate denominator (" + std::string(categoryTitles_[c]) + ");#eta;Stubs",
        100,
        -4.2,
        4.2);
    h_fake_numer_eta_cat_[c] = iBooker.book1D(
        "fake_numer_eta_" + std::string(categoryNames_[c]),
  ...
  for (int i = 0; i < NPerLayerCategories; ++i) {
    ...
    h_fake_numer_pt_perlayer_[i] = iBooker.book1D(
        "fake_numer_pt_" + name, "Fake rate numerator (" + title + ");p_{T} [GeV];Stubs", 100, 0, 50);
    h_fake_denom_pt_perlayer_[i] = iBooker.book1D(
        "fake_denom_pt_" + name, "Fake rate denominator (" + title + ");p_{T} [GeV];Stubs", 100, 0, 50);
  ...
  h_phi_res_barrel_ = iBooker.book1D(
      "phi_res_barrel", "#Delta#phi barrel (genuine);#phi_{stub} - #phi_{TP};Stubs", 200, -0.01, 0.01);
  h_phi_res_endcap_ = iBooker.book1D(
      "phi_res_endcap", "#Delta#phi endcap (genuine);#phi_{stub} - #phi_{TP};Stubs", 200, -0.01, 0.01);
  h_z_res_barrel_ =
      iBooker.book1D("z_res_barrel", "#Deltaz barrel (genuine);z_{stub} - z_{TP} [cm];Stubs", 200, -2, 2);
  h_r_res_endcap_ =
      iBooker.book1D("r_res_endcap", "#DeltaR endcap (genuine);R_{stub} - R_{TP} [cm];Stubs", 200, -2, 2);
  ...
  for (int i = 0; i < NPerLayerCategories; ++i) {
    ...
    h_width_genuine_perlayer_[i] = iBooker.book1D(
        "width_genuine_" + name,
        "|width| genuine (" + title + ");|bend #times r| [cm];Stubs",
        200,
        0,
        0.3);
    h_width_fake_perlayer_[i] = iBooker.book1D(
        "width_fake_" + name,
        "|width| fake (" + title + ");|bend #times r| [cm];Stubs",
        200,
        0,
        0.3);
  }
  ...
  for (int i = 0; i < NPerLayerCategories; ++i) {
    ...
    h_dy_genuine_perlayer_[i] = iBooker.book1D(
        "dy_genuine_" + name,
        "|#Deltay| genuine (" + title + ");|y_{lower} - y_{upper}| [cm];Stubs",
        200,
        0,
        10.0);
    ...
    h_dypull_genuine_perlayer_[i] = iBooker.book1D(
        "dypull_genuine_" + name,
    ...
    h_dypull_fake_perlayer_[i] = iBooker.book1D(
        "dypull_fake_" + name,
        "|#Deltay| pull fake (" + title + ");|#Deltay| / #sigma_{#Deltay};Stubs",
        200,
        0,
        20.0);
  }
  ...
  for (int i = 0; i < NPerLayerCategories; ++i) {
    ...
    h_clusterSize_lower_genuine_perlayer_[i] = iBooker.book1D(
        "clusterSize_lower_genuine_" + name,
        "Cluster size lower genuine (" + title + ");Cluster size;Stubs",
        20,
        0.5,
        20.5);
    h_clusterSize_lower_fake_perlayer_[i] = iBooker.book1D(
        "clusterSize_lower_fake_" + name,
        "Cluster size lower fake (" + title + ");Cluster size;Stubs",
        20,
        0.5,
        20.5);
    h_clusterSize_upper_genuine_perlayer_[i] = iBooker.book1D(
        "clusterSize_upper_genuine_" + name,
        "Cluster size upper genuine (" + title + ");Cluster size;Stubs",
        20,
        0.5,
        20.5);
    h_clusterSize_upper_fake_perlayer_[i] = iBooker.book1D(
        "clusterSize_upper_fake_" + name,
        "Cluster size upper fake (" + title + ");Cluster size;Stubs",
        20,
        0.5,
        20.5);
    h_clusterSizeDiff_genuine_perlayer_[i] = iBooker.book1D(
        "clusterSizeDiff_genuine_" + name,
        "|#DeltaClusterSize| genuine (" + title + ");|size_{lower} - size_{upper}|;Stubs",
        20,
        -0.5,
        19.5);
    h_clusterSizeDiff_fake_perlayer_[i] = iBooker.book1D(
        "clusterSizeDiff_fake_" + name,
        "|#DeltaClusterSize| fake (" + title + ");|size_{lower} - size_{upper}|;Stubs",
        20,
        -0.5,
        19.5);
    h_clusterSize_2d_genuine_perlayer_[i] = iBooker.book2D(
        "clusterSize_2d_genuine_" + name,
        ...
        0.5,
        20.5);
    h_clusterSize_2d_fake_perlayer_[i] = iBooker.book2D(
        "clusterSize_2d_fake_" + name,
        ...
        0.5,
        20.5);
    h_clusterSizeSum_genuine_perlayer_[i] = iBooker.book1D(
        "clusterSizeSum_genuine_" + name,
        "Cluster size sum genuine (" + title + ");cs_{lower} + cs_{upper};Stubs",
        39,
        1.5,
        40.5);
    h_clusterSizeSum_fake_perlayer_[i] = iBooker.book1D(
        "clusterSizeSum_fake_" + name,
    ...
    h_clusterSizeAsymmetry_genuine_perlayer_[i] = iBooker.book1D(
        ...
        "|#Deltacs| / #Sigmacs genuine (" + title +
            ");|cs_{lower} - cs_{upper}| / (cs_{lower} + cs_{upper});Stubs",
        50,
        0.0,
        1.0);
    h_clusterSizeAsymmetry_fake_perlayer_[i] = iBooker.book1D(
        "clusterSizeAsymmetry_fake_" + name,
        "|#Deltacs| / #Sigmacs fake (" + title +
            ");|cs_{lower} - cs_{upper}| / (cs_{lower} + cs_{upper});Stubs",
        50,
        0.0,
        1.0);
    h_clusterSizeMin_genuine_perlayer_[i] = iBooker.book1D(
        "clusterSizeMin_genuine_" + name,
        "Cluster size min genuine (" + title + ");min(cs_{lower}, cs_{upper});Stubs",
        20,
        0.5,
        20.5);
    h_clusterSizeMin_fake_perlayer_[i] = iBooker.book1D(
        "clusterSizeMin_fake_" + name,
  ...
  for (int i = 0; i < NPerLayerCategories; ++i) {
    ...
    h_dPhiDr_fake_perlayer_[i] = iBooker.book1D(
        "dPhiDr_fake_" + name, "dPhiDr fake (" + title + ");dPhiDr [rad/cm];Stubs", 200, -0.005, 0.005);

    h_dPhiDrErr_genuine_perlayer_[i] = iBooker.book1D(
        "dPhiDrErr_genuine_" + name,
        "dPhiDr error genuine (" + title + ");#sigma_{dPhiDr} [rad/cm];Stubs",
        200,
        0,
        0.005);
    h_dPhiDrErr_fake_perlayer_[i] = iBooker.book1D(
        "dPhiDrErr_fake_" + name,
        "dPhiDr error fake (" + title + ");#sigma_{dPhiDr} [rad/cm];Stubs",
        200,
        0,
        0.005);

    h_dPhiDr_signif_genuine_perlayer_[i] = iBooker.book1D(
        "dPhiDr_signif_genuine_" + name,
        "|dPhiDr| / #sigma genuine (" + title + ");|dPhiDr| / #sigma;Stubs",
        200,
        0,
        20.0);
    h_dPhiDr_signif_fake_perlayer_[i] = iBooker.book1D(
        "dPhiDr_signif_fake_" + name,
        "|dPhiDr| / #sigma fake (" + title + ");|dPhiDr| / #sigma;Stubs",
        200,
        0,
        20.0);

    h_bend_genuine_perlayer_[i] = iBooker.book1D(
        "bend_genuine_" + name, "bend genuine (" + title + ");bend [rad];Stubs", 200, -0.005, 0.005);
    ...
    h_ptEst_genuine_perlayer_[i] = iBooker.book1D(
        "ptEst_genuine_" + name,
        "p_{T} estimate genuine (" + title + ");p_{T}^{est} [GeV];Stubs",
        200,
        0,
        100.0);
  ...
  for (int i = 0; i < NBarrelLayers; ++i) {
    ...
    h_stubs_per_phit_genuine_barrel_[i] = iBooker.book1D(
        "stubs_per_phit_genuine_" + layerStr,
        "Stubs per P-hit genuine (" + layerStr + ");N stubs;P-hits",
        20,
        0.5,
        20.5);
    h_stubs_per_phit_fake_barrel_[i] = iBooker.book1D(
        "stubs_per_phit_fake_" + layerStr,
        "Stubs per P-hit all-fake (" + layerStr + ");N stubs;P-hits",
        20,
        0.5,
        20.5);
  }
  ...
  for (int i = 0; i < NPerLayerCategories; ++i) {
    std::string name = perLayerCategoryName(i);
    std::string title = perLayerCategoryTitle(i);
    h_stubs_per_phit_genuine_perlayer_[i] = iBooker.book1D(
        "stubs_per_phit_genuine_" + name,
        "Stubs per P-hit (#geq1 genuine) (" + title + ");N stubs;P-hits",
        20,
        0.5,
        20.5);
    h_stubs_per_phit_fake_perlayer_[i] = iBooker.book1D(
        "stubs_per_phit_fake_" + name,
        "Stubs per P-hit (all fake) (" + title + ");N stubs;P-hits",
        20,
        0.5,
        20.5);
    h_stubs_per_phit_all_perlayer_[i] = iBooker.book1D(
        "stubs_per_phit_all_" + name,
        "Stubs per P-hit (all) (" + title + ");N stubs;P-hits",
        20,
        0.5,
        20.5);
  }

  h_mean_stubs_per_phit_genuine_ = iBooker.bookProfile(
      "mean_stubs_per_phit_genuine",
      ...
      0,
      100);
  h_mean_stubs_per_phit_fake_ = iBooker.bookProfile(
      "mean_stubs_per_phit_fake",
      ...
      0,
      100);
  h_mean_stubs_per_phit_all_ = iBooker.bookProfile(
      "mean_stubs_per_phit_all",
  ...
  for (int i = 0; i < NPerLayerCategories; ++i) {
    ...
    h_width_sim_perlayer_[i] = iBooker.book1D(
        "width_sim_" + name,
        "|#Deltax_{local}| sim (" + title + ");|x_{lower} - x_{upper}| [cm];TP pairs",
        200,
        0,
        0.3);
    h_width_sim_corr_perlayer_[i] = iBooker.book1D(
        "width_sim_corr_" + name,
        "|width| sim parallax-corr (" + title + ");|width_{corr}| [cm];TP pairs",
        200,
        0,
        0.3);
    h_dy_sim_perlayer_[i] = iBooker.book1D(
        "dy_sim_" + name,
        "|#Deltay| sim (" + title + ");|y_{lower} - y_{upper}| [cm];TP pairs",
        200,
        0,
        10.0);
    h_dypull_sim_perlayer_[i] = iBooker.book1D(
        "dypull_sim_" + name,
        "|#Deltay| pull sim (" + title + ");|#Deltay| / #sigma_{#Deltay};TP pairs",
        200,
        0,
        20.0);
  }
...
void Phase2OTValidateVectorHitStyleStub::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  for (uint32_t iStub = 0; iStub < nStubs; ++iStub) {
    ...
    float r = stubsView[iStub].rGlobal();
    h_stub_rz_->Fill(z, r);
    ...
    if (stubTruth[iStub].classification == Genuine) {
      ...
      if (tp.isNonnull()) {
        ...
        float stub_phi = std::atan2(stubsView[iStub].yGlobal(), stubsView[iStub].xGlobal());
        float tp_phi = tp->phi();
    ...
    // === Diagnostic histograms ===
    {
      ...
      if (plIdx >= 0 && plIdx < NPerLayerCategories) {
        float bendVal = stubsView[iStub].bend();
        float rStub = stubsView[iStub].rGlobal();
        float absWidth = std::abs(bendVal * rStub);
        ...
        float ptEst = stubsView[iStub].ptEst();

        uint32_t lowerSoAIdx = hitsView[innerSoAIdx].isLower() ? innerSoAIdx : outerSoAIdx;
        uint32_t upperSoAIdx = hitsView[innerSoAIdx].isLower() ? outerSoAIdx : innerSoAIdx;
        ...
        if (stubTruth[iStub].classification == Genuine) {
          h_width_genuine_perlayer_[plIdx]->Fill(absWidth);
          ...
          h_bend_genuine_perlayer_[plIdx]->Fill(bendVal);
          h_ptEst_genuine_perlayer_[plIdx]->Fill(std::min(ptEst, 99.9f));
          ...
          h_width_fake_perlayer_[plIdx]->Fill(absWidth);
          ...
          h_bend_fake_perlayer_[plIdx]->Fill(bendVal);
          h_ptEst_fake_perlayer_[plIdx]->Fill(std::min(ptEst, 99.9f));
  ...
  // === Stubs-per-P-hit diagnostic histograms ===
  {
    ...
    for (uint32_t iStub = 0; iStub < nStubs; ++iStub) {
      if (stubsView[iStub].stubType() == reco::StubType::PHitOnly)
        continue;
      uint32_t groupId = stubsView[iStub].pHitGroupId();
    ...
    for (const auto& [groupId, stubIndices] : phitToStubs) {
      ...
      float firstZ = stubsView[stubIndices[0]].zGlobal();
